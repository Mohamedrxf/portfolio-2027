...
export function NetworkTopology({
  ...
  useEffect(() => {
    ...
    nodes.forEach((node) => {
      ...
      sub.scale.setScalar(node.type === 'cloud' ? 1.4 : 1);
      ...
      const geo =
        cfg.shape === 'box'
          ...
          : cfg.shape === 'cylinder'
            ? new THREE.CylinderGeometry(0.6, 0.7, 1, 24)
      ...
      const ledCount = node.type === 'server' ? 8 : 4;
    ...
    connections.forEach((conn) => {
      ...
      const line = new THREE.Line(
        g,
        new THREE.LineBasicMaterial({ color: 0x2a3340, transparent: true, opacity: 0.55 })
      );
      group.add(line);
      built.push(line);
  ...
  useEffect(() => {
    ...
    let id: number;
    const tick = () => {
      if (groupRef.current) groupRef.current.rotation.y += rotationSpeed * 0.004;
      const t = performance.now() / 1000;
      packetMeshesRef.current.forEach((p, i) => {
        const curve = curvesRef.current[i];
        if (!curve) return;
        const tt = (t * 0.35 * (1 + (i % 3) * 0.3)) % 1;
        const pt = curve.getPoint(tt);
        p.position.copy(pt);
        p.scale.setScalar(0.5 + Math.sin(tt * Math.PI * 2) * 0.15 + 0.6);
      });
